...
std::string response_to_string(const HttpResponse& response, bool send_content) {
  std::ostringstream oss;
...
HttpRequest requestString_to_requestObject(const std::string& request_string) {
  ...
  if (rpos == std::string::npos) {
    throw std::invalid_argument("Invalid request format : Could not find request start line");
  }
